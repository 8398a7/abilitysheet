  ...
  class Official
    ...
    def normal_get(version)
      ...
      while 1 > 0
        ...
        @agent.page.links.each{ | link | element_get('http://p.eagate.573.jp' + link.href.to_s) if Sheet.exists?(title: link.text.to_s) }
    ...
    def error_title_check(title)
      ...
      return '旋律のドグマ～Miserables～' if title.index '旋律'
      return title
    end
